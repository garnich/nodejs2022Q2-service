...
export class ArtistsService {
    ...
    updateArtist(id: string, payload: UpdateArtistDto): IArtists {
        ...
        const newArtistData = {
          ...
          grammy: payload.grammy !== ArtistsService.atrists[idx].grammy ? payload.grammy: ArtistsService.atrists[idx].grammy,
        };
    ...
    deleteArtist(id: string) {
      const idx = ArtistsService.atrists.findIndex((track: IArtists) => track.id === id);
      ArtistsService.atrists = [...ArtistsService.atrists.slice(0, idx), ...ArtistsService.atrists.slice(idx + 1)];
    }
}
